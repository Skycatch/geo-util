...
describe('StereaUtil', () => {
      ...
      const points = [
        {
          lat: 37,
          lon: -122,
          proj4: '+proj=sterea +lat_0=37 +lon_0=-122 +k_0=1 +x_0=0 +y_0=0'
        },
        {
          lat: 3,
          lon: 4,
          proj4: '+proj=sterea +lat_0=3 +lon_0=4 +k_0=1 +x_0=0 +y_0=0'
        },
        {
          lat: -34.5,
          lon: 34,
          proj4: '+proj=sterea +lat_0=-34.5 +lon_0=34 +k_0=1 +x_0=0 +y_0=0'
        }
      ]
  ...
  describe('StereaUtil#getCoord', () => {
    it('returns the projected coordinate', () => {
      ...
      for (const point of points) {
        const coord = StereaUtil.getCoord(
          point.lat,
          point.lon,
          point.lat0,
          point.lon0
  ...
  describe('StereaUtil#getLatLon', () => {
    it('returns the lat lon coordinate', () => {
      ...
      for (const point of points) {
        const coord = StereaUtil.getLatLon(
          point.northing,
          point.easting,
          point.lat0,
          point.lon0
